/*
 * MD5 implementation, based on Alexander Peslyak's public-domain code
 * (a.k.a. Solar Designer's "md5", placed in the public domain). Used here to
 * verify downloaded files against the md5 archive.org reports in item metadata.
 */
#include "md5.h"
...
/* Read the data block in little-endian byte order. */
#define SET(n)                                                                 \
    (ctx->block[(n)] = (uint32_t)ptr[(n) * 4] |                                \
